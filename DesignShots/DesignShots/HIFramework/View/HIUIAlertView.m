...
@interface HIUIAlertViewStack : NSObject

@property (nonatomic,retain) NSMutableArray *alertViews;

+ (HIUIAlertViewStack *)sharedInstance;

- (void)push:(HIUIAlertView *)alertView;
- (void)pop:(HIUIAlertView *)alertView;
...
@interface HIUIAlertView ()

@end

@implementation HIUIAlertView
...
//        _titleLabel.textColor = HI_RGB(56, 56, 56);
...
//        _messageLabel.textColor = HI_RGB(56, 56, 56);
...
//    [button setTitleColor:HI_RGB(56, 56, 56) forState:UIControlStateNormal];
//    [button setTitleColor:HI_RGB(86, 86, 86) forState:UIControlStateHighlighted];
    [button setTitleColor:HI_RGB(10, 110, 210) forState:UIControlStateNormal];
    [button setTitleColor:HI_RGB(10, 110, 240) forState:UIControlStateHighlighted];
...
- (void)show
{
    [[HIUIAlertViewStack sharedInstance] push:self];
}
...
- (void)dismiss:(id)sender animated:(BOOL)animated
{
    ...
    if ([[[HIUIAlertViewStack sharedInstance] alertViews] count] == 1) {
        if (animated) {
    ...
    [UIView animateWithDuration:(animated ? 0.2 : 0) animations:^{
        ...
        [[HIUIAlertViewStack sharedInstance] pop:self];
        [self.view removeFromSuperview];
    }];
    if (self.clickBlock) {
        BOOL cancelled = NO;
...
+ (instancetype)showAlertWithTitle:(NSString *)title
                       ...
                       cancelTitle:(NSString *)cancelTitle
                        completion:(HIUIAlertViewCompletionBlock)completion
{
    HIUIAlertView *alertView = [[self alloc] initWithTitle:title
                                                 message:message
                                             cancelTitle:cancelTitle
                                              otherTitle:nil
                                             contentView:nil
...
@implementation HIUIAlertViewStack

+ (instancetype)sharedInstance
{
    static HIUIAlertViewStack *_sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _sharedInstance = [[HIUIAlertViewStack alloc] init];
...
- (void)push:(HIUIAlertView *)alertView
{
    ...
    for (HIUIAlertView *av in self.alertViews) {
        if (av != alertView) {
            [av hide];
        }
    }
...
- (void)pop:(HIUIAlertView *)alertView
{
    ...
    HIUIAlertView *last = [self.alertViews lastObject];
    if (last) {
        [last showInternal];
    }
}
