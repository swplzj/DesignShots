...
+ (BOOL)setObject:(id)object forKey:(id)key
{
    ...
    NSAssert(!object || (object && data), @"HIKeychain failed to encode object for key '%@', error: %@", key, error);
    ...
    if (data) {
        ...
        if (status != errSecSuccess) {
            ERROR(@"HIKeychain failed to store data for key '%@', error: %ld", key, (long)status);
            return NO;
        }
    } else {
        ...
        if (status != errSecSuccess) {
            ERROR(@"HIKeychain failed to delete data for key '%@', error: %ld", key, (long)status);
            return NO;
        }
    }
...
+ (NSData *)dataForKey:(id)key
{
    ...
    if (status != errSecSuccess && status != errSecItemNotFound) {
        ERROR(@"HIKeychain failed to retrieve data for key '%@', error: %ld", key, (long)status);
    }
...
{
    NSData * data = [HIKeychain dataForKey:key];
    ...
    if (data) {
        ...
        if (!object) {
            ERROR(@"HIKeychain failed to decode data for key '%@', error: %@", key, error);
        }
        return object;
    } else {
        //no value found
...
{
    return [HIKeychain setObject:nil forKey:key];
}
