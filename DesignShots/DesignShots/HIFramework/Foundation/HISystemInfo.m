...
#define RUNS_NUMBER_KEY @"HI_RUNS_NUMBER_KEY"

@implementation HISystemInfo

+(void) load
{
    NSNumber * number = [HIUserDefaults objectForKey:RUNS_NUMBER_KEY];
    
    if (!number) {
        [HIUserDefaults setObject:[NSNumber numberWithInt:1] forKey:RUNS_NUMBER_KEY];
    }else{
        [HIUserDefaults setObject:[NSNumber numberWithInt:([number intValue] + 1)] forKey:RUNS_NUMBER_KEY];
    }
}
...
+ (NSString *)deviceUUID
{
    ...
    Class openUDID = NSClassFromString( @"HIUDID" );
...
{
#if (TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR)
    return [HISystemInfo isScreenSize:CGSizeMake(320, 480)];
#else
    return NO;
#endif
}
...
{
#if (TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR)
    return [HISystemInfo isScreenSize:CGSizeMake(640, 960)];
#else
    return NO;
#endif
}
...
{
#if (TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR)
    return [HISystemInfo isScreenSize:CGSizeMake(640, 1136)];
#else
    return NO;
#endif
}
...
{
#if (TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR)
    return [HISystemInfo isScreenSize:CGSizeMake(768, 1024)];
#else
    return NO;
#endif
}
...
{
#if (TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR)
    return [HISystemInfo isScreenSize:CGSizeMake(1536, 2048)];
#else
    return NO;
#endif
}
...
+ (NSString *)deviceName
{
    ...
    if (HI_NSSTRING_IS_INVALID(name)) {
        return @"";
    }
...
+ (NSString *) systemVersion
{
    ...
    if (HI_NSSTRING_IS_INVALID(device)) {
        return @"";
    }
...
{
    NSString *device = [HISystemInfo platformType];
...
+ (NSString *)totalDiskSpace
{
    ...
    return [HISystemInfo memoryFormatter:space];
}

+ (NSString *)freeDiskSpace
{
    long long freeSpace = [[[[NSFileManager defaultManager] attributesOfFileSystemForPath:NSHomeDirectory() error:nil] objectForKey:NSFileSystemFreeSize] longLongValue];
    return [HISystemInfo memoryFormatter:freeSpace];
}

+ (NSString *)usedDiskSpace
{
    return [HISystemInfo memoryFormatter:[HISystemInfo usedDiskSpaceInBytes]];
}
...
{
    long long usedSpace = [HISystemInfo totalDiskSpaceInBytes] - [HISystemInfo freeDiskSpaceInBytes];
    return usedSpace;
}
